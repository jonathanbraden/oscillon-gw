...
\end{equation}
with $\phi_0$ a parameter characterizing the amplitude of the field oscillations inside the oscillon, and $\sigma$ the width of the oscillon profile. \jb{This assumes only one scale over which the profile varies.}
...
\jb{How does the $H_0$ from the $\eta$ integral change this argument.  Seems to indicate that when the oscillons are the dominant contribution, the signal actually decreases a $N_{\rm osc}^{-1/2}$, but there are some $a$'s floating around that I need to be careful with.}
