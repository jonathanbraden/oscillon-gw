...
\jb{Somewhere mention that we include modes beyond Nyquist just to show what the effects there can be, not because they are meaningful.}
...
\begin{equation}
  \label{eqn:amplitude-stat-phase-general}
...
The result~\eqref{eqn:amplitude-stat-phase-general} is a central result of our analytic analysis, and can be widely applied to \emph{any} long-lived field profile oscillating at a single characteristic frequency $\omega$.
To make further progress, additional information about the spatial field profile (encoded in $\tijNoT$) is needed.
In many cases, the profile will have exhibit scaling properties, leading to a scaling of the stress-energy tensor itself.
Additional progress can be made in this case without an explicit form for $\tijNoT$ which we will now explore.
...
\jb{State that this may be very difficult to disentangle from a bunch of unresolved astrophysical sources in a real detector, so pushing the signal amplitude above the sensitivity of a GW observatory isn't the key.  The multi-peaked, or single peaked signal morphology is different and it seems more plausible that it can be separated, but those signals don't arise from long-lived sources (and therefore don't arise just by ``running our unresolved simulations that we clearly don't understand'' longer.)}
...
\begin{figure}
  \includegraphics[width=3.375in]{{{form-factor-converge-len-2osc-floor}}}
  \includegraphics[width=3.375in]{{{form-factor-converge-len-2osc-center}}}
  \includegraphics[width=3.375in]{{{form-factor-converge-len-2osc-welch}}}
  \caption{An illustration of how $F_2$ approaches the infinite volume limit when sampling in a finite box.  We plot the ``angle averaged'' $F_2$ estimated on a lattice by sampling at the discrete lattice wavenumbers and averaging within equally spaced bins in absolute wavenumber.  To show convergence to the infinite volume limit, we take take the two oscillon positions to be separated by $L_{\rm 16}\Delta r = $\jb{??} along the $x$-axis.  We then vary the overall lattice size by repeatedly doubling the number of lattice sites per linear dimension as well as the overall side length, so that $dx$ and $\Delta r$ are both held fixed.  The red line is the continuum limit~\eqref{eqn:} obtained by analytically computing the angular integrals for the two object system.  The vertical grey band indicates modes on the 3D lattice beyond the 1D Nyquist wavenumber.  In order to illustrate the effects of various binning schemes we show the result by binning by the floor of the wavenumber (\emph{Top Row}), centered bins (\emph{Center Row}), and using a Welch filter (\emph{Bottom Row}). \jb{Add separations of $0.3$ and $0.7$ for right column} \jb{Fix the horrific tick locations.}  \jb{Add the grey band for beyond Nyquist modes.}}
\end{figure}

In the more general case of $N$ objects with individual locations $x_n$ are drawn uniformly in the interval $[0,L]$ and $k = 2\pi i / L$, then each individual phase factor is a Gaussian Random Deviate with unit variance \jb{or 2?}.
