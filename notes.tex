...
\jb{Check sign of phase shift.  Improve this to capture the time-dependence in non-oscillatory integrand better for a precise determination of the overall amplitude.}
...
\jb{Add window function for early times.  Note that if we integrate for too long, the $\cos\tau$ and $\sin\tau$ terms might also contribute instead of simply integrating to zero on average.  However, in this limit we might also need to use more accurate Green's functions and include the other corrections from the Hubble in $\rho_{\rm GW}$.}
...
\begin{equation}
  \frac{1}{(2\pi)^3}\int d\Omega \sum_{i,j}\left|\tijNoT\right|^2 = \phi_0^2\sigma^2 \tilde{F}_0 \, .
\end{equation}
...
\begin{equation}
  \Omega_{\rm GW} = \mathcal{N}\frac{1}{6M_P^4H_0^2V}a^{3w-1}\frac{\pi\omega^2}{2H_0}\left(\frac{k_{\rm pivot}}{2\omega}\right)^{\frac{9}{2}+\frac{3}{2}w}\left(\frac{k}{k_{\rm pivot}}\right)^{\frac{5}{2}+\frac{3}{2}w}\int d\Omega \sum_{i,j}\left|\tij(k,0)\right|^2
\end{equation}
...
\end{equation}
\jb{Note that this is symmetric (underlying phase factor gets an overall sign) in a finite box evaluated at the harmonics of the box.  This symmetry under $\Delta x \to L - \Delta x$ must be there since the DFT implicitly acts on the periodic extension.  Natural to put this in discrete sampling section.}
...
\jb{Now add some scalings for $\Omega_{\rm GW}$ given a collection of oscillons.  Show that it only scales with the number of oscillons if they are a subdominant part of the energy density.  Factor it into a piece that depends on the oscillon profile (i.e. the potential), and a piece dependent on the density (i.e. initial conditions and subsequent evolution).  Consider case of $\omega\sigma \sim 1$ and only a single width characterizing oscillon (i.e. no flat tops).}
\begin{equation}
...
\jb{How does the $H_0$ from the $\eta$ integral change this argument}
...
\end{align}
\jb{Rewrite the $g^2$ term in some more obvious form.}
...
\jb{Remove this, just used as an algebra check}
For
\begin{equation}
