...
# Plotting routines for analytic calculations of the spherically symmetric Gaussian
import numpy as np
...
Tkxy_o2 = lambda x,y,dx : -(1./dx**2)*np.exp(-0.25*(x**2+y**2))*np.sin(0.5*x*dx)*np.sin(0.5*y*dx)*np.exp(-0.5*dx**2)
Tkxx_o2 = lambda x,y,dx : (0.5/dx**2)*np.exp(-0.25*(x**2+y**2))*(np.cos(x*dx)-np.exp(-dx**2))
Tkxy_exact = lambda x,y,dx : -0.25*np.exp(-0.25*(x**2+y**2))*x*y
Tkxx_exact = lambda x,y,dx : 0.5*np.exp(-0.25*(x**2+y**2))*(1.-0.5*x**2)
Tkxy_diff = lambda x,y,dx : -0.25*np.exp(-0.25*(x**2+y**2))*((4./dx**2)*np.sin(0.5*x*dx)*np.sin(0.5*y*dx)*np.exp(-0.5*dx**2)-x*y)
Tkxx_diff = lambda x,y,dx : 0.5*np.exp(-0.25*(x**2+y**2))*(np.cos(x*dx)/dx**2-np.exp(-dx**2)/dx**2 - 1. + 0.5*x**2)
...
def plot_Tkij_slice(func,dx):
    x = np.linspace(-6.,6.,201); y = np.linspace(-6.,6.,201)
    xv,yv = np.meshgrid(x,y)

    fig, ax = plt.subplots()
    cnt = ax.contourf(xv,yv,func(xv,yv,dx),51,cmap='coolwarm',zorder=-20)
    ax.set_rasterization_zorder(-10)
    
    ax.set_xlabel(r'$\sigma k_x$'); ax.set_ylabel(r'$\sigma k_y$')
...
def plot_Tij_slice(func,dx):
    ...
    cb = fig.colorbar(cnt,fraction=0.05,pad=0.02)
